...
"Story","Configure Environment Inventory Structure","EPIC 1 — Platform Foundation","Create AWX-compatible inventories (QA/UAT/Perf). Mandatory vars: execution_root, artifact_backup_root, artifactory_base_url. AC: No artifact names as groups, variables present.","2","High","AWX-Orchestration","Wave-1,Phase-1","US-001","US-004,US-019"
"Story","Build AWX Execution Environment Definition","EPIC 1 — Platform Foundation","Build custom EE container. Tools: ansible-core, python, curl, jq, tar, gzip, coreutils. AC: Smoke test (curl/jq/python) passes in AWX.","5","Highest","Execution-Environment","Wave-1,Docker,AWX","","EPIC-2,EPIC-4,EPIC-6"
"Story","Configure AWX Credential Framework","EPIC 1 — Platform Foundation","Create CRED_EXECUTION_HOST_SSH, CRED_ARTIFACTORY, CRED_SMTP. AC: Auth connectivity validated.","3","Medium","Security","Wave-1,Credentials","US-002,US-003","US-020,US-034"
"Story","Define Runtime Manifest Schema Contract","EPIC 2 — Runtime Input & Validation","Define contract for environment, execution_mode, resume_execution, artifact_name, run_as_user. AC: Schema documented and approved.","3","High","Manifest-Framework","Wave-1,Validation","US-001","US-006,US-007"
"Story","Implement Manifest Validation Role","EPIC 2 — Runtime Input & Validation","Role to validate schema, duplicates, and runtime users. AC: Rejects root or unknown users. Rejects duplicates.","5","High","Ansible-Roles","Wave-1,Validation","US-005,US-003","US-007,US-040"
"Story","Create Manifest Validation Playbook","EPIC 2 — Runtime Input & Validation","AWX entry-point. Generates RUN_ID, creates workspace, persists manifest. AC: Validates input before execution.","5","High","AWX-Orchestration","Wave-1,Playbook","US-006,US-003","US-034,US-035"
"Story","Design Checkpoint Schema and Run-State Contracts","EPIC 3 — Checkpoint / Resume Framework","Define checkpoint.yml and run_state.yml. AC: All runtime phases (cleanup, deploy, etc.) represented.","3","Medium","Checkpoint-Recovery","Wave-1,Resume","US-005","US-010,US-012"
"Story","Implement Checkpoint Manager Role","EPIC 3 — Checkpoint / Resume Framework","Role to update phase states and load checkpoints. AC: Phase completion persists to filesystem.","5","High","Ansible-Roles","Wave-1,Checkpoint","US-009,US-003","US-012,US-019"
"Story","Implement Resume Dispatcher Playbook","EPIC 3 — Checkpoint / Resume Framework","Logic to skip completed/approved artifacts and resume at failure phase. AC: Correct phase resolution after failure.","5","High","AWX-Orchestration","Wave-1,Resume","US-010,US-011","US-037"
"Story","Create Artifact Metadata Framework","EPIC 4 — Artifact Execution Framework","Metadata schema for artifact-specific scripts (cleanup, sanity, jobs). AC: No hardcoded artifact logic in roles.","5","Highest","Artifact-Framework","Wave-2,Metadata","US-013","US-014,US-019"
"Story","Build Artifact Validation Role","EPIC 4 — Artifact Execution Framework","Executes comparison scripts. Normalizes exits: 0=match, 1=diff, 2=fail. AC: Differences do not fail the artifact.","8","High","Ansible-Roles","Wave-2,Comparison","US-013,US-017","US-019,US-026A"
"Story","Create Core Artifact Pipeline Playbook","EPIC 4 — Artifact Execution Framework","Primary lifecycle engine. Orchestrates Cleanup -> Deploy -> Execute -> Validation. AC: Supports resume and prod/release branches.","13","Highest","AWX-Orchestration","Wave-2,Core-Pipeline","Wave-2-Roles","US-035,US-036"
"Story","Build Artifact Deployment Role","EPIC 5 — Deployment Framework","Standardized deployment: Backup -> Download -> Checksum -> Install. AC: Backup exists before replacement.","5","High","Ansible-Roles","Wave-2,Deployment","US-021,US-013","US-019"
"Story","Build Sequential Job Execution Engine","EPIC 6 — Job Execution Engine","Ordered background execution. AC: Job2 starts only after Job1 success. Runtime user enforced.","5","High","Job-Execution","Wave-2,Sequential","US-003,US-013","US-024,US-025"
"Story","Build Parallel Job Execution Engine","EPIC 6 — Job Execution Engine","Bounded concurrency (max 2). AC: Process count does not exceed limit. Aggregate results.","8","High","Job-Execution","Wave-2,Parallel","US-003,US-013","US-024,US-025"
"Story","Implement Timeout and Process Monitoring Framework","EPIC 6 — Job Execution Engine","Supervision logic. SIGTERM/SIGKILL runaway processes. AC: No orphan processes after timeout.","8","High","Runtime-Control","Wave-2,Timeout","US-022,US-023","US-025"
"Story","Implement Validation Report Generation Framework","EPIC 7 — Validation Reporting Framework","HTML Report generation using Jinja. AC: Standardized report includes comparison summary and metadata.","5","Medium","Validation-Framework","Wave-3,Reporting","US-018","US-028A"
"Story","Implement Artifact Comparison Script Execution Framework","EPIC 7 — Validation Reporting Framework","Added logic to locate and execute artifact-specific comparison scripts. AC: Passes correct output paths.","5","High","Ansible-Roles","Wave-3,Comparison","US-018","US-026"
"Story","Publish Validation Reports as AWX Job Artifacts","EPIC 7 — Validation Reporting Framework","Copy reports to /runner/artifacts/ for UI access. AC: Link resolvable from AWX Job.","3","Medium","AWX-Orchestration","Wave-3,Artifacts","US-026,US-003","US-031"
"Story","Configure AWX Notification Templates","EPIC 8 — AWX Notification Governance","Native AWX SMTP templates (Failure, Approval, Validation). AC: Test mail delivered in 2 mins.","3","Medium","AWX-Orchestration","Wave-3,AWX","US-004","US-030,US-039"
"Story","Implement Sequential Workflow Template","EPIC 9 — AWX Workflow Orchestration","Orchestration: Validate -> Execute -> Approval -> Resume. AC: Artifact 2 waits for approval.","8","High","AWX-Orchestration","Wave-3,Sequential","US-034,US-038","US-047"
"Story","Implement Parallel Workflow Template","EPIC 9 — AWX Workflow Orchestration","Orchestration: Validate -> Parallel Dispatch (5). AC: Consolidated summary notification.","5","High","AWX-Orchestration","Wave-3,Parallel","US-034,US-032","US-048"
"Story","Implement Runtime User Governance Validation","EPIC 10 — Security & Runtime Governance","Validate run_as_user against allowlist. AC: Rejects root or unknown users.","3","Highest","Security","Wave-4,Governance","US-006,US-002","US-047"
"Story","Configure sudo / Privilege Escalation Governance","EPIC 10 — Security & Runtime Governance","Explicit sudoers mapping. AC: Block unauthorized escalation (e.g., to oracle).","5","Highest","Security","Wave-4,Sudo","Admin-Access","US-047"
"Story","Execute Sequential End-to-End Certification Campaign","EPIC 11 — Testing & Certification","Certify 8 scenarios including failure and resume. AC: All scenarios pass and matched spec.","8","High","Testing","Wave-4,Certification","All-Wave-3","US-054"
"Story","Create Operations Runbook","EPIC 12 — Documentation & Operational Readiness","Day-2 support: failure diagnosis, resume rules, log locations. AC: Support review complete.","5","Medium","Documentation","Wave-4,Operations","Wave-4-Certification","US-054"
"Story","Create Final Architecture & Operational Handover Package","EPIC 12 — Documentation & Operational Readiness","Final package for sign-off. AC: Stakeholder approval for production release.","5","High","Governance","Wave-4,Handover","US-047,US-052","Go-Live"
...
# Master Directives
- **Sub-Task Delegation:** You MUST break the project into smaller chunks and delegate them to child agents. Give each child strict instructions on what folder/files to look at, and tell them exactly what output file to generate (e.g., `./parts/db_section.md`).
- **Prose & Diagram Rules for Children:** When delegating, instruct the child agents that they must write deep narrative prose and save Mermaid diagrams as separate `.mermaid` files.
- **Two-Pass Publishing:** You (the Orchestrator) are responsible for final assembly. You will stitch the text parts together, publish to Confluence (Pass 1), and then patch the Mermaid XML macros into the published page (Pass 2).

# Orchestrated Execution Workflow
You have been provided `<repo_name>` and `<confluence_space>`. Execute these steps sequentially:

1. **Initialization:** - Create a directory called `./tech_spec_parts`.
   - Use the Confluence MCP to read the **Master Technical Specification document template**. 
   - Create a `./tech_spec_parts/00_master_draft.md` file and copy the template headings into it.

2. **Phase 1: High-Level Discovery (You do this):** - Use the Bitbucket MCP to look at the root directory (`pom.xml`, `build.gradle`, config folders). 
   - Identify the primary application modules and database folders.

3. **Phase 2: Delegation (Spin up Child Agents):**
   - **Delegate Task A (Properties):** Spin up a child agent to analyze the config files and write findings to `./tech_spec_parts/01_config.md`.
   - **Delegate Task B (Database):** Spin up a child agent to analyze JPA/SQL scripts. Instruct it to write prose to `./tech_spec_parts/02_db.md` and save the ERD to `./tech_spec_parts/erd.mermaid`.
   - **Delegate Task C (Modules):** For each core module identified, spin up a child agent. Instruct it to analyze the module's Java code, write prose to `./tech_spec_parts/03_module_[X].md`, and save the sequence diagram to `./tech_spec_parts/seq_[X].mermaid`.
   - *Wait for all child tasks to complete.*

4. **Phase 3: Assembly (You do this):**
   - Read all the markdown files generated by the child agents in `./tech_spec_parts/`.
   - Stitch them together into `./tech_spec_parts/00_master_draft.md` under the correct template headings. Ensure placeholders (e.g., `[[PLACEHOLDER_ERD]]`) are correctly positioned.

5. **Phase 4: Publishing:**
   - **Pass 1:** Use the Confluence MCP to publish the assembled `00_master_draft.md` to `<confluence_space>` using the naming convention: `[Service Name] - Master Technical Specification`.
   - **Pass 2:** Read the `.mermaid` files generated by the children. Wrap them in Confluence `<ac:structured-macro ac:name="mermaid-macro"...>` XML format. Use the Confluence MCP to update the page, replacing the placeholders with the XML payloads.

6. **Cleanup:** Delete the `./tech_spec_parts` directory.
======
