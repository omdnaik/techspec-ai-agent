...
Locate the MCP Server initialization logic (likely inside cli.py under the mcp_server command, or wherever [GraphCode MCP] Performing incremental sync on startup... is printed).
Completely delete the logic that triggers this incremental sync. The MCP server must start up completely idle and wait for client connections. It should never invoke the AST parser or ingestion scripts.
​Action 2: Sever the Memgraph/Neo4j Connection
In the same initialization sequence, locate where the server prints Connecting to Memgraph at localhost:7687.
Delete all initialization code related to the Neo4j/Memgraph driver, connection pools, or environment variables (MEMGRAPH_HOST, MEMGRAPH_PORT, etc.).
Note: Do not initialize a global Kùzu connection here yet. Our multi-tenant architecture dictates that Kùzu connections will be created dynamically per-session later.

-----


Context: We are finalizing Wave 1 of the Codebase RAG project. We have aligned on a strict CI/CD architecture where the Kùzu graph database is treated as an immutable snapshot generated by Jenkins. We will not be supporting live incremental graph updates from local IDE changes. Therefore, the background File Watcher daemon is an architectural violation and must be completely removed.
​Action 1: Eradicate the File Watcher
Search the ingestion entry point (likely cli.py, main.py, or similar) for any watchdog library imports, observer classes, or file system event handlers.
Completely delete all of this logic. The script must no longer monitor directories for file modifications.
​Action 2: Enforce Single-Run Execution
Ensure the main execution flow runs Pass 1 through Pass 4, successfully completes the database flush to Kùzu, prints the final success log, and then immediately and cleanly exits the Python process (Return Code 0). Remove any while True: time.sleep() loops that were keeping the daemon alive.
​Action 3: Dependency Cleanup
Remove the watchdog package from requirements.txt, pyproject.toml, or Pipfile, as it is no longer an active dependency.
